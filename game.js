		...
		for (const box of document.querySelectorAll(`.c${i}`)) {
			if (box.nextElementSibling) {
				if (
					...
					parseInt(box.nextElementSibling.children[0].innerText) ==
						parseInt(box.children[0].innerText)
				) {
					movements = true;
	...
	for (let i = 1; i < 4; i++) {
		...
		if (nR) {
			for (let j = 0; j < r.children.length; j++) {
				if (
					r.children[j].children[0].innerText != "" &&
					parseInt(r.children[j].children[0].innerText) ==
						parseInt(nR.children[j].children[0].innerText)
				) {
					movements = true;
					r.children[j].children[0].innerText =
						parseInt(r.children[j].children[0].innerText) * 2;
					nR.children[j].children[0].innerText = "";
				}
			}
		}
		...
		for (const box of document.querySelectorAll(`.c${i}`)) {
			if (box.previousElementSibling) {
				if (
					...
					parseInt(box.previousElementSibling.children[0].innerText) ==
						parseInt(box.children[0].innerText)
				) {
					movements = true;
					box.children[0].innerText = parseInt(box.children[0].innerText) * 2;
